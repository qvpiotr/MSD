...
public class Board extends JComponent implements MouseInputListener, ComponentListener {
    ...
    public void iteration() {
        ...
        for (int x = 1; x < points.length - 1; ++x)
            for (int y = 1; y < points[x].length - 1; ++y)
                if(points[x][y].isPedestrian && !points[x][y].blocked && !points[x][y].neighbors.isEmpty()){

                    PointsToMove.add(points[x][y]);
                }
        assert PointsToMove != null;
    ...
    private void initialize(int length, int height) {
        ...
        if(MOORE) {
            for (int x = 1; x < points.length - 1; ++x) {
                for (int y = 1; y < points[x].length - 1; ++y) {
                    if(points[x+1][y].type != 1) points[x][y].addNeighbor(points[x+1][y]);
                    if(points[x-1][y].type != 1) points[x][y].addNeighbor(points[x-1][y]);
                    if(points[x][y+1].type != 1) points[x][y].addNeighbor(points[x][y+1]);
                    if(points[x][y-1].type != 1) points[x][y].addNeighbor(points[x][y-1]);
                    if(points[x+1][y+1].type != 1) points[x][y].addNeighbor(points[x+1][y+1]);
                    if(points[x-1][y+1].type != 1) points[x][y].addNeighbor(points[x-1][y+1]);
                    if(points[x+1][y-1].type != 1) points[x][y].addNeighbor(points[x+1][y-1]);
                    if(points[x-1][y-1].type != 1) points[x][y].addNeighbor(points[x-1][y-1]);
                }
